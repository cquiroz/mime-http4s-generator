...
final case class Mime(mainType: String, secondaryType: String, descr: MimeDescr) {
  ...
  def toTree(mediaTypeClass: ClassSymbol): Tree =
    if (descr.extensions.isEmpty) {
      LAZYVAL(valName, mediaTypeClass) := NEW(mediaTypeClass, LIT(mainType), LIT(secondaryType), compressibleRef, binaryRef)
    } else {
      LAZYVAL(valName, mediaTypeClass) := NEW(mediaTypeClass, LIT(mainType), LIT(secondaryType), compressibleRef, binaryRef, extensions)
    }
}
...
object MimeLoader {
  ...
  def toTree(mainType: String, objectName: String, mediaTypeClassName: String)(mimes: List[Mime]): (List[Tree], String) = {
    def subObject(partial: Boolean, listVal: String, objectName: String, mimes: List[Mime]): Tree = {
      val all: Tree = (LAZYVAL(listVal, ListClass TYPE_OF TYPE_REF(REF(mediaTypeClassName)))) := LIST(mimes.map(m => REF((m.valName))))
    ...
    if (mimes.length <= maxSizePerSection) {
      ...
      val reducedAll = subObjects.zipWithIndex.map{ case (_, i) => REF(s"part_$i")}.foldLeft(NIL){ (a, b) => (a LIST_::: b) }
      val all: Tree = (LAZYVAL("all", ListClass TYPE_OF TYPE_REF(REF(mediaTypeClassName))) := reducedAll)
      val objectPartsDefinition = (OBJECTDEF(s"${objectName}_parts") withFlags(PRIVATEWITHIN("http4s"))):= BLOCK(subObjects.map(_._1))
      val objectDefinition = (OBJECTDEF(objectName) withParents(subObjects.map(x => s"${objectName}_parts.${x._2}"))):= BLOCK(all)
      (List(objectPartsDefinition, objectDefinition), mainType)
    }
  }

  def coalesce(l: List[(List[Tree], String)], topLevelPackge: String, objectName: String, mediaTypeClassName: String): Tree = {
    ...
    (((OBJECTDEF(objectName) withFlags(PRIVATEWITHIN(privateWithin))) := BLOCK(List(all, compressible, uncompressible, binary, notBinary) ::: l.flatMap(_._1)))) inPackage(topLevelPackge) withComment("Autogenerated file, don't edit")
  }
