...
trait MimeTypes {
  val MediaTypeClass = RootClass.newClass("MediaType")
...
object MimeLoader extends App with MimeTypes {
  ...
  def toTree(mimes: List[Mime]): Tree = {
    val all: Tree = (VAL("all", ListClass TYPE_OF TYPE_REF(REF("MediaType")))) := LIST(mimes.map(m => REF((m.valName))))
    val vals = mimes.map(_.toTree)
    val allVals = all :: vals
    ((OBJECTDEF("MimeDB") := BLOCK(IMPORT("org.http4s.MediaType") :: allVals))) inPackage("org.http4s")
  }

  def toFile(f: Path): IO[Unit] = IO.pure(())

  mimes.map(toTree).map(x => println(treeToString(x))).run.unsafeRunSync
}
